...
import { PDFRequestService } from "../../types/requestsInterfaces";
...
const PDFParticularDetail = (data: PDFRequestService) => {
  const { request, deceased, types, sizes, colors, companies } = data;

  const date = new Date(request.date);

  const currentDay = date.getDay() < 10 ? "0" + date.getDay() : date.getDay();
      ...
      <Page size="A4" style={{ paddingBottom: "4vh" }}>
        <View>
          ...
          <View style={styles.container}>
            ...
            <View>
              {companies.find((c) => {
                c.name == request.company;
              }) ? (
                <View style={styles.row}>
                  <View style={styles.items}>
                    <Text style={styles.itemTitle}>Empresa:</Text>
                    <Text style={styles.itemText}> {request.company}</Text>
                  </View>
                </View>
              ) : (
                <View style={styles.row}>
                  <View style={styles.items}>
                    <Text style={styles.itemTitle}>Particular:</Text>
                    <Text style={styles.itemText}> {request.company}</Text>
                  </View>
                </View>
              )}
              <View style={styles.row}>
                <View style={styles.items}>
                  <Text style={styles.itemTitle}>
                    Apellido y Nombre del fallecido:
                  </Text>
              ...
              <View style={styles.row}>
                ...
                <View style={styles.items}>
                  ...
                  <Text style={styles.itemText}>
                    {" "}
                    {new Date(deceased.dod).toLocaleDateString("es")}
                  </Text>
                </View>
                <View style={styles.items}>
              ...
              <View style={styles.row}>
                <View style={styles.items}>
                  ...
                  <Text style={styles.itemText}>
                    {new Date(deceased.dob).toLocaleDateString("es")}
                  </Text>
                </View>
                <View style={styles.items}>
              ...
              <View style={styles.row}>
                <View style={styles.items}>
                  ...
                  <Text style={styles.itemText}>
                    {" "}
                    {decomposeId(request.id_coffin_group)}
                  </Text>
                </View>
              </View>
              <View style={styles.row}>
              ...
              <View style={styles.row}>
                <View style={styles.items}>
                  ...
                  <Text style={styles.itemText}>
                    {" "}
                    {deceased.news_paper_name}
                  </Text>
                </View>
              </View>
              <View style={styles.lastRow}>
                <Text style={styles.pharagraph}>
                  Por la presente doy conformidad por un servicio de Sepelio
                  efectuado conforme a las condiciones de la póliza contratada
                  en Servicios Sociales Instituto del Norte S.A.
                </Text>
              </View>
              <View style={styles.row}>
                <Text style={styles.pharagraph}>
                  Por lo tanto la Empresa ha cumplido en forma satisfactoria con
                  todo lo convenido y no admitirá reclamo alguno en el futuro.
                </Text>
              </View>
              <View style={styles.lastRow}>
                <View style={styles.footer}>
                  <View style={styles.itemSignature}>
                    ...
                    <Text style={styles.itemText}>
                      N° de Documento:_______________________________
                    </Text>
                    <Text style={styles.itemText}>
...
export default PDFParticularDetail;
