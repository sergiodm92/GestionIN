...
async def get_coffin_stock_id_service(id):
    try:
        ...
        coffin_stock = db.collection(
            'coffin_stock').document(id).get().to_dict()
...
async def post_transfer(transfer):
    try:
        # Crea el nuevo documento en Firestore con los datos de add
        doc_ref_origin = db.collection(
            'coffin_stock').document(transfer.id_origin)
        doc_ref_origin_get = doc_ref_origin.get()
        doc_ref_destiny = db.collection(
            'coffin_stock').document(transfer.id_destiny)
        doc_ref_destiny_get = doc_ref_destiny.get()
        place_destiny = parse_id_coffin(transfer.id_destiny)[
            'place']  # traer el place usando slice
        if doc_ref_origin_get.exists:
            ...
            if doc_ref_destiny_get.exists:
                ...
                model = CoffinStock(
                    id_coffin=transfer.id_destiny, units=transfer.units, place=place_destiny)
